...
def rand_target_loc():
    ...
    # MODIFIED TO ACCOUNT FOR OBSTACLE EXTRUSIONS
    # y = np.random.uniform(low=0.08, high=0.5)
    # if np.random.randint(0, 2) == 0:
    #     y = -y
    # x = np.random.uniform(low = 0.3, high = 0.8)
    # z = np.random.uniform(low=0.65,  high = 0.95)
...
if __name__ == '__main__':
    ...
    dataset_type = '3D' # '2D': using the LASA 2D "joint" dataset, '3D': using the 3D Franka Environment
    which_data   = 1    # 0: lasa dataset, 1: 2D franka environment, 2: Random RBF generated environment 
