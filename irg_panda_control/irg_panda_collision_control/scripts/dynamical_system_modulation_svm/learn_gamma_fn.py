...
def get_gamma(position, classifier, max_dist, reference_points, dimension = 2):
    """
    Cite: https://github.com/epfl-lasa/dynamic_obstacle_avoidance_linear
    """
    pos_shape     = position.shape
...
# def get_gamma_multiRef(position, classifier, max_dist, reference_point):
#     """
#     Cite: https://github.com/epfl-lasa/dynamic_obstacle_avoidance_linear
#     """
#     pos_shape     = position.shape
#     nb_ref_points = len(reference_point)
#     svm_bias = classifier.intercept_
...
def create_obstacles_from_data(data, label, cluster_eps=0.01, cluster_min_samples=10, label_free=0, label_obstacle=1, plot_raw_data=False, gamma_svm=0, c_svm=1000, cluster_labels=[]):
    """
    Cite: https://github.com/epfl-lasa/dynamic_obstacle_avoidance_linear
    ...
    if cluster_labels == []:    
        ...
        for oo in range(n_obstacles):
            obstacle_points = data[:,cluster_labels==oo]
...
def draw_contour_map(classifier, max_dist, reference_points, fig=None, ax=None, show_contour=True, gamma_value=False, normal_vecs = None, show_vecs=True, show_plot = True, grid_limits_x=[0,1], grid_limits_y=[0,1], grid_size=50, data=[]):
    ...
    if len(reference_points) > 1:
        for oo in range(len(reference_points)):
            ...
            ax.plot([reference_point[0]], [reference_point[1]], 'k+')
    else: 
        ax.plot([reference_points[0]], [reference_points[1]], 'k+')   
